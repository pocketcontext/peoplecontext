#!/usr/bin/env python3
"""Checks of the PeopleContext container image. Python 3 standard library and the docker CLI only.

  python3 docker/smoke.py smoke   --image IMAGE   start as ONCE does, provision a user, run the skill client, stop, start again
...
def superuser_token(base, email, password):
    ...
    check(True, 'superuser login with PEOPLECONTEXT_SUPERUSER_EMAIL and PEOPLECONTEXT_SUPERUSER_PASSWORD')
    return secret(body['token'])


def provision_user(base, token, email, password):
    status, _, body = http('POST', base + '/api/collections/agents/records', token=token,
    ...
    status, _, _ = http('POST', base + '/api/collections/hr_members/records', token=token, body={'account': body['id']})
    check(status == 200, 'synthetic account explicitly granted HR role')
    return body['id']
...
class Client:
    ...
    def __init__(self, base, email, password, home):
        ...
        self.env = {**os.environ, 'HOME': str(self.home), 'PEOPLECONTEXT_URL': base, 'PEOPLECONTEXT_AGENT_EMAIL': email, 'PEOPLECONTEXT_AGENT_PASSWORD': password}
...
def write_batch(client, user_id):
    employee, salary = client.run('newid').strip(), client.run('newid').strip()
    reply = json.loads(client.run('batch', '-', stdin=json.dumps([
        {'method': 'POST', 'url': '/api/collections/employees/records', 'body': {'id': employee, 'name': 'Synthetic employee'}},
        {'method': 'POST', 'url': '/api/collections/compensation/records', 'body': {'id': salary, 'employee': employee, 'annual_salary_minor': 123400, 'currency': 'EUR', 'effective_date': '2026-01-01 00:00:00.000Z'}},
    ])))
    check([entry.get('status') for entry in reply] == [200, 200], 'atomic synthetic employee and compensation creation')
    return employee, salary


def check_records(client, employee, salary):
    rows = client.sql(f"SELECT e.name, c.annual_salary_minor FROM employees e JOIN compensation c ON c.employee = e.id WHERE e.id = '{employee}' AND c.id = '{salary}'")
    check(rows == [['Synthetic employee', 123400]], 'authorized HR snapshot and role survived')
...
def once_env(extra=None):
    ...
    env = {
        'BASE_URL': 'https://people.example.test', 'SECRET_KEY_BASE': secret(secrets.token_hex(32)), 'DISABLE_SSL': 'true', 'NUM_CPUS': '2',
        ...
        'PEOPLECONTEXT_SUPERUSER_EMAIL': 'operator@example.test', 'PEOPLECONTEXT_SUPERUSER_PASSWORD': secret('-' + secrets.token_urlsafe(24)),
        'PEOPLECONTEXT_GOOGLE_CLIENT_ID': 'image-test.apps.googleusercontent.com',
        'PEOPLECONTEXT_GOOGLE_CLIENT_SECRET': secret(secrets.token_urlsafe(24)),
...
def smoke(image, tmp, run_id):
    ...
    token = superuser_token(base, env['PEOPLECONTEXT_SUPERUSER_EMAIL'], env['PEOPLECONTEXT_SUPERUSER_PASSWORD'])
    ...
    check(settings['rateLimits']['enabled'] is True, "rate limits are enabled by the image's default PEOPLECONTEXT_RATE_LIMITS=true")
    ...
    check(collection['oauth2']['providers'][0]['clientId'] == env['PEOPLECONTEXT_GOOGLE_CLIENT_ID'], 'Google client ID matches the environment')
    check(env['PEOPLECONTEXT_GOOGLE_CLIENT_SECRET'] not in json.dumps(collection), 'the collection API does not return the Google secret')
    ...
    check(collection['authToken']['duration'] == 604800 and collection['authRule'] == 'disabled = false', 'seven-day tokens and disabled-account rule')
...
def config(image, tmp, run_id):
    ...
    expect_startup_error(image, 'a superuser email without a password', {'LITESTREAM_DISABLED': 'true', 'PEOPLECONTEXT_SUPERUSER_EMAIL': 'operator@example.test'},
                         ['PEOPLECONTEXT_SUPERUSER_PASSWORD'])

    expect_startup_error(image, 'a Google client ID without a secret',
                         {'LITESTREAM_DISABLED': 'true', 'PEOPLECONTEXT_GOOGLE_CLIENT_ID': 'image-test.apps.googleusercontent.com'},
                         ['PEOPLECONTEXT_GOOGLE_CLIENT_SECRET'])
    expect_startup_error(image, 'a Google secret without a client ID',
                         {'LITESTREAM_DISABLED': 'true', 'PEOPLECONTEXT_GOOGLE_CLIENT_SECRET': secret(secrets.token_urlsafe(24))},
                         ['PEOPLECONTEXT_GOOGLE_CLIENT_ID'])
...
def restore(image, tmp, run_id):
    network, minio, bucket = f'tc-drill-{run_id}', f'tc-drill-minio-{run_id}', 'peoplecontext-drill'
    ...
    superuser_token(base, env['PEOPLECONTEXT_SUPERUSER_EMAIL'], env['PEOPLECONTEXT_SUPERUSER_PASSWORD'])

    step('a write immediately before docker stop must reach the replica through the shutdown sync')
    late = json.loads(client.run('create', 'employees', json.dumps({'name': 'Written before stop'})))['id']
    stop(second)
    ...
    check_records(client, project, issue)
    rows = client.sql(f"SELECT name FROM employees WHERE id = '{late}'")
    check(rows == [['Written before stop']], f'the employee written before the stop was restored (rows: {rows})')
    check_logs(third)
    stop(third)
    check_logs(minio)
...
def main():
    ...
    failed = True
    with tempfile.TemporaryDirectory(prefix='peoplecontext-image-') as tmp:
        try:
