...
serve() {
	...
	unset LITESTREAM_ACCESS_KEY_ID LITESTREAM_SECRET_ACCESS_KEY \
		AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY PEOPLECONTEXT_SUPERUSER_PASSWORD
...
if [ -n "${PEOPLECONTEXT_GOOGLE_CLIENT_ID:-}" ] && [ -z "${PEOPLECONTEXT_GOOGLE_CLIENT_SECRET:-}" ]; then
	die "PEOPLECONTEXT_GOOGLE_CLIENT_ID requires PEOPLECONTEXT_GOOGLE_CLIENT_SECRET"
elif [ -n "${PEOPLECONTEXT_GOOGLE_CLIENT_SECRET:-}" ] && [ -z "${PEOPLECONTEXT_GOOGLE_CLIENT_ID:-}" ]; then
	die "PEOPLECONTEXT_GOOGLE_CLIENT_SECRET requires PEOPLECONTEXT_GOOGLE_CLIENT_ID"
fi
...
if [ -n "${PEOPLECONTEXT_SUPERUSER_EMAIL:-}" ] && [ -n "${PEOPLECONTEXT_SUPERUSER_PASSWORD:-}" ]; then
	log "upserting the superuser from PEOPLECONTEXT_SUPERUSER_EMAIL"
	# shellcheck disable=SC2046 # see serve
	if ! "$SERVER" superuser upsert $(app_flags) -- "$PEOPLECONTEXT_SUPERUSER_EMAIL" "$PEOPLECONTEXT_SUPERUSER_PASSWORD"; then
		die "superuser upsert failed"
	fi
elif [ -n "${PEOPLECONTEXT_SUPERUSER_EMAIL:-}" ]; then
	die "PEOPLECONTEXT_SUPERUSER_EMAIL is set but PEOPLECONTEXT_SUPERUSER_PASSWORD is missing"
elif [ -n "${PEOPLECONTEXT_SUPERUSER_PASSWORD:-}" ]; then
	die "PEOPLECONTEXT_SUPERUSER_PASSWORD is set but PEOPLECONTEXT_SUPERUSER_EMAIL is missing"
fi
