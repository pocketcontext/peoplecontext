...
HOST = "people.pocketcontext.com"
IMAGE = "ghcr.io/pocketcontext/peoplecontext:latest"
...
def deploy():
    matches = people_containers()
    if len(matches) != 1:
        raise RuntimeError("Expected exactly one PeopleContext container; refusing an ambiguous update")
    container = matches[0]
    if container["Config"]["Image"] != IMAGE and not re.fullmatch(
            r"ghcr\.io/pocketcontext/peoplecontext@sha256:[0-9a-f]{64}", container["Config"]["Image"]):
        raise RuntimeError("PeopleContext image does not match the fixed deployment target")
    ...
    try:
        ...
        if state["Running"] or state["ExitCode"] != 0 or state.get("OOMKilled"):
            raise RuntimeError("PeopleContext did not stop gracefully; refusing to replace it")
        ...
        try:
            ...
            print("PeopleContext recovery failed; operator intervention required", file=sys.stderr)
        raise
...
def main():
    ...
    with open("/run/lock/deploy-peoplecontext.lock", "a") as lock:
