...
function settings(app) {
  ...
  const header = env("PEOPLECONTEXT_TRUSTED_PROXY_HEADER");
  if (header) {
  ...
  const limits = env("PEOPLECONTEXT_RATE_LIMITS");
  if (limits === "true") {
    ...
    console.log("deploy: PEOPLECONTEXT_RATE_LIMITS must be true or false; rate limit settings left alone");
  }
}

function googleOAuth(app) {
  const clientId = String($os.getenv("PEOPLECONTEXT_GOOGLE_CLIENT_ID") || "");
  const clientSecret = String($os.getenv("PEOPLECONTEXT_GOOGLE_CLIENT_SECRET") || "");
  if (!clientId && !clientSecret) return;
  if (!clientId.trim() || !clientSecret.trim()) {
    throw new Error("PEOPLECONTEXT_GOOGLE_CLIENT_ID and PEOPLECONTEXT_GOOGLE_CLIENT_SECRET must be set together");
  }
  if (/\s/.test(clientId) || /\s/.test(clientSecret)) {
    throw new Error("PeopleContext Google OAuth credentials must not contain whitespace");
  }
  try {
    // Fresh databases configure OAuth in the application migration after agents exists.
    let users;
    try { users = app.findCollectionByNameOrId("agents"); } catch (_) { return; }
    ...
    throw new Error("Could not apply PeopleContext Google OAuth configuration; server startup stopped");
  }
}
