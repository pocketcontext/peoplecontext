function isUser(record) {
  return record && record.collection().name === "agents";
}
...
function request(e) {
  ...
  if (!e.auth && e.request.header.get("Authorization")) {
    throw new UnauthorizedError("The PeopleContext session is invalid or expired.");
  }
  if (isUser(e.auth)) {
    // Re-read so this guard also applies when an earlier handler supplied auth.
    const current = e.app.findRecordById("agents", e.auth.id);
    if (current.getBool("disabled") || current.tokenKey() !== e.auth.tokenKey()) {
      throw new ForbiddenError("This PeopleContext account is disabled or its session was revoked.");
    }
  }
  return e.next();
}
...
function remove() {
  throw new ForbiddenError("PeopleContext accounts cannot be deleted; disable the account to preserve employee links and history.");
}
...
function realtime(e) {
  ...
  if (!auth) return;
  if (isUser(auth)) {
    const current = e.app.findRecordById("agents", auth.id);
