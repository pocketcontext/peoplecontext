...
Configuration comes from three environment variables:
  ...
  PEOPLECONTEXT_AGENT_EMAIL     email of an account in the `agents` collection
  PEOPLECONTEXT_AGENT_PASSWORD  password of that account (optional with Google login)
...
def config(names=ENV[:2]):
    ...
    if parsed.scheme not in ('http', 'https') or not parsed.hostname or parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise Fail(2, 'PEOPLECONTEXT_URL must be an HTTP(S) URL without credentials, query, or fragment')
    if parsed.scheme == 'http' and parsed.hostname not in ('localhost', '127.0.0.1', '::1'):
        raise Fail(2, 'Use HTTPS for a remote PeopleContext server')
...
def cache_file(cfg):
    ...
    return Path(base) / 'peoplecontext' / (key + '.json')
...
def send(cfg, method, path, body=None, token=None, timeout=TIMEOUT):
    ...
    try:
        ...
        if 300 <= status < 400:
            raise Fail(1, f'HTTP {status}: the server redirects to {error.headers.get("Location")}. Set PEOPLECONTEXT_URL to the final address.')
...
def auth_session(cfg, data, method):
    ...
    if (not isinstance(token, str) or not token or not isinstance(record, dict) or record.get('collectionName') != 'agents'
        ...
        raise Fail(1, 'Authentication returned an unexpected identity; no session saved. Check PEOPLECONTEXT_AGENT_EMAIL.')
...
def google_login(cfg, port=8765, timeout=180):
    ...
    if status != 200 or not oauth.get('enabled') or not provider:
        raise Fail(1, 'Google OAuth is not enabled on this PeopleContext server.')
...
def ordinary_body(body):
    if any(key.strip('+-') == 'work_email' for key in body):
        raise Fail(2, 'employee work_email changes require an operator; ordinary HR writes cannot assign identity')
    return body
...
def run(args):
    ...
    if args.command in ('create', 'update'):
        body = ordinary_body(record_body(args.json))
        if args.collection not in WRITE_COLLECTIONS:
            raise Fail(2, 'writes accept only ordinary HR collections; account and authority changes require an operator')
        ...
        for entry in requests:
            ...
            if method not in ('POST', 'PATCH') or not isinstance(url, str) or not re.fullmatch(r'/api/collections/(employees|compensation|personal_details|hr_notes)/records(?:/[a-z0-9]{15})?', url):
...
def parse(argv):
    ...
    add('whoami', 'print the account id, name, and server URL')
