# PeopleContext image for Basecamp ONCE: HTTP on port 80, GET /up, all state under /storage.
...
ENV PEOPLECONTEXT_RATE_LIMITS=true
VOLUME /storage
EXPOSE 80
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/entrypoint.sh"]

ARG REVISION=unknown
LABEL org.opencontainers.image.title="PeopleContext" \
      org.opencontainers.image.description="Private HR filtered snapshots, Google Workspace authentication, Litestream replication" \
      org.opencontainers.image.source="https://github.com/pocketcontext/peoplecontext" \
